...
const SignUp = () => {
	  ...
	  // ID 형식 검사 (최소 6글자 이상, 영어와 숫자를 모두 포함하며 특수문자는 포함 불가)
        ...
        setIdValid({
            ...
            msg: idRegex.test(newId) && !containsSpecialCharacter
                ...
                : <span style={{ color: 'red' }}>최소 6글자이며, 영어, 숫자를 모두 포함하며 특수문자는 사용할 수 없습니다.</span>
        });
  ...
  const handlePasswordChange = (e) => {
    ...
    // 비밀번호 형식 검사 (영어, 숫자, 특수문자가 모두 포함된 4자리)
        const isValid = /^(?=.*[a-zA-Z])(?=.*\d)(?=.*[!@#$%^&*()_+{}\[\]:;<>,.?~\\/-]).{4,}$/.test(newPassword);
  ...
  return (
	  <>
		  <div className='container_1'>
			  ...
			  <div className='Login_1'>
				  <div className='input_1'>
					  <InputGroup className="mb-3">
						  ...
						  <Form.Control type={passwordVisible ? 'text' : 'password'} placeholder="비밀번호" aria-label="Username" aria-describedby="basic-addon1" onChange={handlePasswordChange} />
					  </InputGroup>
					  <FontAwesomeIcon icon={faEye} size="xl" onClick={togglePasswordVisibility} className={Myinfomodule['eye-icon']} />
				  </div>
			  </div>
			  <div className='msgAlert'>
				  {!passwordValid && (
					  <div className='password-warning text-danger' style={{ fontWeight: 'bold' }}>
						  비밀번호는 최소 4자 이상이어야 하며, 숫자와 영어, 특수 문자를 모두 포함해야 합니다.
					  </div>
				  )}
			  </div>
			  ...
			  {!emailValid && (
				  <div className='email-warning text-danger' style={{ fontWeight: 'bold' }}>올바른 이메일 형식이 아닙니다.</div>
			  )}
